...
for zero_mean in [True, False]:
    for name in sorted(methods.keys()):
        for weeks in [2, 3, 4, 8, 32, 52, 52 * 2, 52 * 3]:
            ...
            items.append(
                [generate_sample_with_weeks(weeks, zero_mean, methods[name]), label]
            )

assert len(items) <= len(robot_names)

for name, (df, label) in zip(robot_names, items):
    print(f"Sending {name} - {label}")
    send_in_chunks(df, num_chunks=num_chunks, email=os.environ["EMAIL"], name=name)
